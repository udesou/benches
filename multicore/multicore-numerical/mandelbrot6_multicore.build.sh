#!/usr/bin/env bash
# mandelbrot6_multicore.build.sh — requires domainslib
set -euo pipefail
...
if [[ ! -f "${BENCH_DIR}/mandelbrot6_multicore.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/mandelbrot6_multicore.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
"${OCAMLFIND}" ocamlopt -O3 -package domainslib -linkpkg "${BENCH_DIR}/mandelbrot6_multicore.ml" -o "${OUT}"
chmod +x "${OUT}"
