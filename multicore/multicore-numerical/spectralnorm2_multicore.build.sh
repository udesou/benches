#!/usr/bin/env bash
# spectralnorm2_multicore.build.sh — requires domainslib
set -euo pipefail
...
if [[ ! -f "${BENCH_DIR}/spectralnorm2_multicore.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/spectralnorm2_multicore.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
"${OCAMLFIND}" ocamlopt -O3 -package domainslib -linkpkg "${BENCH_DIR}/spectralnorm2_multicore.ml" -o "${OUT}"
chmod +x "${OUT}"
