#!/usr/bin/env bash
# binarytrees5_multicore.build.sh — requires domainslib
set -euo pipefail
...
if [[ ! -f "${BENCH_DIR}/binarytrees5_multicore.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/binarytrees5_multicore.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
"${OCAMLFIND}" ocamlopt -O3 -package domainslib -linkpkg "${BENCH_DIR}/binarytrees5_multicore.ml" -o "${OUT}"
chmod +x "${OUT}"
