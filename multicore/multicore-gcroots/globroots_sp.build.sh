#!/usr/bin/env bash
# globroots_sp.build.sh — builds the globroots_sp benchmark binary.
#
# Source: sandmark benchmarks/multicore-gcroots/
# Deps: domainslib (for Chan), C stub (globrootsprim.c); OCaml >= 5 required.
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release globroots_sp.exe
cp "${BUILD_DIR}/default/globroots_sp.exe" "${OUT}"
chmod +x "${OUT}"
