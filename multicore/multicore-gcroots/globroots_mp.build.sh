#!/usr/bin/env bash
# globroots_mp.build.sh — builds the globroots_mp benchmark binary.
#
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release globroots_mp.exe
cp "${BUILD_DIR}/default/globroots_mp.exe" "${OUT}"
chmod +x "${OUT}"
