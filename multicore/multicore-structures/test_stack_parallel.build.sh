...
if [[ ! -f "${BENCH_DIR}/test_stack_parallel.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/test_stack_parallel.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cd "${BENCH_DIR}"
"${OCAMLOPT}" -O3 -I +unix unix.cmxa treiber_stack.ml test_stack_parallel.ml -o "${OUT}"
chmod +x "${OUT}"
