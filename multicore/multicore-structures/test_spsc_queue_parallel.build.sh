#!/usr/bin/env bash
# test_spsc_queue_parallel.build.sh
set -euo pipefail
...
if [[ ! -f "${BENCH_DIR}/test_spsc_queue_parallel.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/test_spsc_queue_parallel.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cd "${BENCH_DIR}"
"${OCAMLOPT}" -O3 -I +unix unix.cmxa spsc_queue.ml test_spsc_queue_parallel.ml -o "${OUT}"
chmod +x "${OUT}"
