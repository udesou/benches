#!/usr/bin/env bash
# test_queue_sequential.build.sh
set -euo pipefail
...
if [[ ! -f "${BENCH_DIR}/test_queue_sequential.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/test_queue_sequential.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cd "${BENCH_DIR}"
"${OCAMLOPT}" -O3 -I +unix unix.cmxa ms_queue.ml test_queue_sequential.ml -o "${OUT}"
chmod +x "${OUT}"
