...
if [[ ! -f "${BENCH_DIR}/test_queue_parallel.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/test_queue_parallel.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cd "${BENCH_DIR}"
"${OCAMLOPT}" -O3 -I +unix unix.cmxa ms_queue.ml test_queue_parallel.ml -o "${OUT}"
chmod +x "${OUT}"
