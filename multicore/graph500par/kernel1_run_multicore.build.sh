...
if [[ ! -f "${BENCH_DIR}/kernel1_run_multicore.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/kernel1_run_multicore.ml" >&2; exit 1
fi
...
"${OCAMLFIND}" ocamlopt -O3 -package domainslib,unix -linkpkg \
  graphTypes.ml sparseGraph.ml generate.ml kernel1Par.ml kernel1_run_multicore.ml -o "${OUT}"
chmod +x "${OUT}"
