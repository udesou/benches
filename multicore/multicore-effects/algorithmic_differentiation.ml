(* Reverse-mode Algorithmic differentiation using effect handlers.
   ...
   (adapted from sandmark multicore-effects/algorithmic_differentiation.ml for OCaml 5.2+ Effect API)
 *)
...
module F : sig
  ...
  let run f =
    ignore (Effect.Deep.match_with f ()
      ...
      ; effc = fun (type a) (eff : a Effect.t) ->
          ...
          | Add (a, b) ->
              Some (fun (k : (a, _) Effect.Deep.continuation) ->
                ...
                ignore (Effect.Deep.continue k x);
                a.d <- a.d +. x.d;
                b.d <- b.d +. x.d;
                x)
          | Mult (a, b) ->
  ...
  let (+.) a b = Effect.perform (Add (a, b))
  let ( *. ) a b = Effect.perform (Mult (a, b))
end
