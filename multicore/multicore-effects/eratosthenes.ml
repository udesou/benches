...
(* adapted from sandmark multicore-effects/eratosthenes.ml for OCaml 5.2+ Effect API *)
...
type _ Effect.t +=
  | Spawn : (pid -> unit) -> pid Effect.t
  | Yield : unit Effect.t
  | Send  : pid * message -> unit Effect.t
  | Recv  : pid -> message option Effect.t

let spawn p = Effect.perform (Spawn p)
let yield () = Effect.perform Yield

(* Communication primitives *)
let send pid data =
  Effect.perform (Send (pid, data));
  yield ()

let rec recv pid =
  match Effect.perform (Recv pid) with
  | Some m -> m
  | None -> yield (); recv pid
...
let run main () =
  ...
  let rec spawn f =
    pid := 1 + !pid;
    Effect.Deep.try_with f !pid
      { effc = fun (type a) (eff : a Effect.t) ->
          match eff with
          | Yield ->
              Some (fun (k : (a, _) Effect.Deep.continuation) ->
                enqueue (fun () -> Effect.Deep.continue k ()); dequeue ())
          | Spawn p ->
              Some (fun (k : (a, _) Effect.Deep.continuation) ->
                enqueue (fun () -> Effect.Deep.continue k !pid); spawn p)
          | _ -> None }
  in
  spawn main
