#!/usr/bin/env bash
# grammatrix_multicore.build.sh — requires domainslib + unix
set -euo pipefail
...
if [[ ! -f "${BENCH_DIR}/grammatrix_multicore.ml" ]]; then
  echo "Source not found: ${BENCH_DIR}/grammatrix_multicore.ml" >&2; exit 1
fi

mkdir -p "$(dirname "${OUT}")"
cd "${BENCH_DIR}"
"${OCAMLFIND}" ocamlopt -O3 -package domainslib,unix -linkpkg \
  utls.ml grammatrix_multicore.ml -o "${OUT}"
chmod +x "${OUT}"
