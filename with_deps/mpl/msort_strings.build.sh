...
# Input: inputs/words64.txt (37 MB, bundled); pass absolute path via -f.
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release msort_strings.exe
cp "${BUILD_DIR}/default/msort_strings.exe" "${OUT}"
chmod +x "${OUT}"
