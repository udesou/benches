...
# Deps: domainslib (direct + via Forkjoin); OCaml >= 5 required.
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release raytracer.exe
cp "${BUILD_DIR}/default/raytracer.exe" "${OUT}"
chmod +x "${OUT}"
