...
# Input: inputs/words.txt (590 KB, bundled); pass absolute path via -f.
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release tokens.exe
cp "${BUILD_DIR}/default/tokens.exe" "${OUT}"
chmod +x "${OUT}"
