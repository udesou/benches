...
if [[ -z "${DUNE_BIN:-}" ]]; then
  ...
  echo "Examples:" >&2
  echo "  opam install dune" >&2
  echo "  sudo apt install dune" >&2
  echo "  build_env: { DUNE_BIN: /path/to/dune }" >&2
  exit 1
fi

if [[ ! -f "${BENCH_DIR}/dune" ]]; then
  echo "dune file not found in ${BENCH_DIR}" >&2
  exit 1
fi
...
for tool in ocaml ocamlc ocamlopt ocamldep; do
  if [[ ! -x "${OCAML_BIN_DIR}/${tool}" ]]; then
    echo "Expected ${tool} at ${OCAML_BIN_DIR}/${tool}, but it was not found/executable." >&2
    exit 1
  fi
done

# Ensure dune resolves compiler tools from the runtime selected by YAML config.
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release markbench.exe
cp "${BUILD_DIR}/default/markbench.exe" "${OUT}"
chmod +x "${OUT}"
