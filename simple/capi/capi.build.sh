#!/usr/bin/env bash
# capi.build.sh — builds the capi benchmark binary.
#
# Source: sandmark benchmarks/simple-tests/capi.ml + ocamlcapi.{ml,c}
# Measures OCaml→C FFI call overhead (alloc vs noalloc, 0/few/many args).
# No external packages required; uses dune (foreign_stubs) for the C stub.
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release capi.exe
cp "${BUILD_DIR}/default/capi.exe" "${OUT}"
chmod +x "${OUT}"
