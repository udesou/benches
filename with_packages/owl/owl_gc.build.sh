#!/usr/bin/env bash
# owl_gc.build.sh — builds the owl_gc benchmark binary.
#
# Source: sandmark benchmarks/owl/owl_gc.ml
# Deps: owl-base (pure OCaml; no CBLAS/LAPACK required)
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release owl_gc.exe
cp "${BUILD_DIR}/default/owl_gc.exe" "${OUT}"
chmod +x "${OUT}"
