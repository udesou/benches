...
#
# Required opam packages: zarith num
set -euo pipefail
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release zarith_fact.exe
cp "${BUILD_DIR}/default/zarith_fact.exe" "${OUT}"
chmod +x "${OUT}"
