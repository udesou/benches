...
#
# Required opam packages: yojson camlp-streams
#
...
_opam_auto_install() {
  local pkgs="yojson camlp-streams"
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release ydump.exe
cp "${BUILD_DIR}/default/ydump.exe" "${OUT}"
chmod +x "${OUT}"
