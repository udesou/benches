#!/usr/bin/env bash
# test_lwt.build.sh — builds the valet/test_lwt benchmark binary.
#
# Source: sandmark benchmarks/valet/
# Deps: uuidm, ocplib-endian, react, lwt (OxCaml-incompatible: lwt.unix)
set -euo pipefail
...
_OPAM_PKGS="uuidm ocplib-endian react lwt"
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release test_lwt.exe
cp "${BUILD_DIR}/default/test_lwt.exe" "${OUT}"
chmod +x "${OUT}"
