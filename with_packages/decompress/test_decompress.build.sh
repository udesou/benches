...
#
# Required opam packages: decompress bigstringaf checkseum
set -euo pipefail
...
_OPAM_PKGS="decompress bigstringaf checkseum"
...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release test_decompress.exe
cp "${BUILD_DIR}/default/test_decompress.exe" "${OUT}"
chmod +x "${OUT}"
