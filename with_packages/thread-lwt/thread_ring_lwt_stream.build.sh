...
mkdir -p "$(dirname "${OUT}")"
"${DUNE_BIN}" build --root "${BENCH_DIR}" --build-dir "${BUILD_DIR}" --profile release thread_ring_lwt_stream.exe
cp "${BUILD_DIR}/default/thread_ring_lwt_stream.exe" "${OUT}"
chmod +x "${OUT}"
